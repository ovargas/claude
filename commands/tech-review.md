...
1. **Map the codebase:**
   ...
   **If `--deep` was passed:** Spawn **virtual-team:codebase-analyzer** agent: "Map the full directory structure of [scope], identify major patterns (file organization, naming conventions, error handling, state management, data access), and find all documentation and decision records. Show 2-3 examples of each pattern found."
...
Review security directly — check for common issues (auth bypass, injection, data exposure). Only spawn **virtual-team:security-reviewer** agent if `--deep` was passed and security is the primary concern.
